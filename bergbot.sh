...
Versao()
{
	...
	[sS] ) echo -e "\033[1mIniciando atualização...\033[0m";
		echo $(sudo apt upgrade -y)
                echo
                echo -e "\033[1;32mSistema atualizado!\033[0m";;
	[nN] ) echo -e "\033[1mAté logo :)\033[0m";
		true;;
	* ) echo -e "\033[1;31mResposta inválida\033[0m";;
        esac
}
...
Mousepad()
{
        echo -e "\033[1mEstou iniciando o mousepad mas confesso que prefiro o VI\033[0m"
        sleep 1
        mousepad
}
...
Nmap()
{
        ...
        *'1'*)
        ...
        ;;
        *'2'*)
                echo -e "\033[1mcalma ae que eu vou chamar meu parceiro nmap.\033[0m"
                sleep 2
                read -p "Qual é o alvo?: " HOST
                sleep 1
...
case $PERGUNTA in *'ola'* | *'Ola'*)
        
        Ola
        ;;
esac

case $PERGUNTA in *'windows'*)
        
        Windows
        ;;
esac

case $PERGUNTA in *'ajuda'*)
        
        Ajuda
        ;;
esac

case $PERGUNTA in *'serviço'* | *'servico'*)
        
        Servico
        ;;
esac

case $PERGUNTA in *'conexao'* | *'conexão'* | *'conec'* | *'ping'*)

        Conexao
        ;;
esac

case $PERGUNTA in *'atualiz'*)
        
        Atualizar
        ;;
esac

case $PERGUNTA in  *'versao'*)
        
        Versao
        ;;
esac

case $PERGUNTA in *'vulnera'* | *'nmap'* | *'porta'*)
        
        Nmap
        ;;
esac

case $PERGUNTA in *'criado'* | *'criou'* | *'criador'*)
        
        Criador
        ;;
esac

case $PERGUNTA in *'namor'* | *'namorada'*)

        Namoro
        ;;
esac

case $PERGUNTA in *'trem'*)
        
        Trem
        ;;
esac

case $PERGUNTA in *'banner'*)
        
        Banner
        ;;
esac

case $PERGUNTA in *'diretorio'* | *'gobuster'*)
        
        Gobuster
        ;;
esac

case $PERGUNTA in *'manha'* | *'tarde'* | *'noite'* | *'hora'*)
                
        Horario
        ;;
esac

case $PERGUNTA in *'calc'* | *'matematica'* | *'conta'*)
...
case $PERGUNTA in *'editar'* | *'nota'* | *'texto'*)
